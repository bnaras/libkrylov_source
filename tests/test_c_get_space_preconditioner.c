...
int main() {
  ...
  error = ckrylov_get_space_preconditioner(index, value, value_len);
  CHECK(error == CKRYLOV_OK);
  CHECK(strncmp(value, "n", 1) == 0);

  error = ckrylov_set_space_preconditioner(index, "d", 1);
  CHECK(error == CKRYLOV_OK);

  error = ckrylov_get_space_preconditioner(index, value, value_len);
  CHECK(error == CKRYLOV_OK);
  CHECK(strncmp(value, "d", 1) == 0);

  error = ckrylov_set_space_preconditioner(index, "q", 1);
