...
int main() {
  ...
  error = ckrylov_get_space_orthonormalizer(index, value, value_len);
  CHECK(error == CKRYLOV_OK);
  CHECK(strncmp(value, "o", 1) == 0);

  error = ckrylov_set_space_orthonormalizer(index, "n", 1);
  CHECK(error == CKRYLOV_OK);
  ...
  error = ckrylov_set_space_orthonormalizer(index, "q", 1);
