program test_real_shifted_linear_equation_solve_projected

    use kinds, only: IK, RK
    use errors, only: OK
    use options, only: config_t
    use testing, only: near_real_mat
    use krylov, only: real_shifted_linear_equation_t, real_ortho_orthonormalizer_t
    implicit none

    type(real_shifted_linear_equation_t) :: equation
    type(real_ortho_orthonormalizer_t) :: orthonormalizer
    type(config_t) :: config
    real(RK) :: vectors(3_IK, 2_IK), rayleigh(2_IK, 2_IK), shifts(2_IK), basis_rhs(2_IK, 2_IK), &
...
end program test_real_shifted_linear_equation_solve_projected
