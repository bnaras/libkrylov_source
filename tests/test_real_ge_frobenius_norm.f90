program test_real_ge_frobenius_norm

    use kinds, only: IK, RK
    use testing, only: near_real_num
    use linalg, only: real_ge_frobenius_norm
    implicit none
    ...
    norm = real_ge_frobenius_norm(a, 3_IK, 3_IK)
    if (norm /= near_real_num(4.0_RK)) stop 1

end program test_real_ge_frobenius_norm
