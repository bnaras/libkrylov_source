...
int main() {
  ...
  char_t key1[] = "logical";
  error = ckrylov_set_space_logical_option(index, key1, strlen(key1), true);
  CHECK(error == CKRYLOV_OK);

  value = ckrylov_get_space_logical_option(index, key1, strlen(key1));
  CHECK(value);

  char_t key2[] = "missing";
  value = ckrylov_get_space_logical_option(index, key2, strlen(key2));
  CHECK(!value);
