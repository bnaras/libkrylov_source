...
int main() {
  ...
  real_t sylvester_b_1[] = {1.0, 1.0, 1.0, 1.0}, sylvester_b_2[4];
  error =
      ckrylov_set_real_space_sylvester_b(index, solution_dim, sylvester_b_1);
  CHECK(error == CKRYLOV_OK);

  error =
      ckrylov_get_real_space_sylvester_b(index, solution_dim, sylvester_b_2);
  CHECK(error == CKRYLOV_OK);
  CHECK(near_real(sylvester_b_1, sylvester_b_2, solution_dim * solution_dim));
