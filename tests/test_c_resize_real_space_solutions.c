...
int main() {
  ...
  int_t full_dim = 100, solution_dim = 1, basis_dim = 3;
  index = ckrylov_add_space(kind, strlen(kind), structure, strlen(structure),
                            equation, strlen(equation), full_dim, solution_dim,
                            basis_dim);
  CHECK(index == 1);

  error = ckrylov_resize_real_space_solutions(index, 3);
  CHECK(error == CKRYLOV_OK);

  length = ckrylov_get_space_solution_size(index);
  CHECK(length == 300);

  index = 2;
  error = ckrylov_resize_real_space_vectors(index, 2);
