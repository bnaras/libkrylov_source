program test_convergence_get_solution_convergence

    use kinds, only: IK, RK
    use errors, only: OK, NO_ITERATIONS, NO_SUCH_SOLUTION, NOT_CONVERGED
    ...
    status = convergence%get_solution_convergence(1_IK)
    ...
    status = convergence%get_solution_convergence(2_IK)
    if (status /= OK) stop 1

    status = convergence%get_solution_convergence(3_IK)
    if (status /= NO_SUCH_SOLUTION) stop 1

end program test_convergence_get_solution_convergence
