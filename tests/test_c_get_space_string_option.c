...
int main() {
  ...
  char_t key1[] = "string", value1[] = "a";
  error = ckrylov_set_space_string_option(index, key1, strlen(key1), value1,
                                          strlen(value1));
  CHECK(error == CKRYLOV_OK);

  error = ckrylov_get_space_string_option(index, key1, strlen(key1), value,
                                          value_len);
  CHECK(error == CKRYLOV_OK);
  CHECK(strncmp(value, value1, strlen(value1)) == 0);

  length = ckrylov_length_space_string_option(index, key1, strlen(key1));
  CHECK(length == 1);

  char_t key2[] = "missing";
  error = ckrylov_get_space_string_option(index, key2, strlen(key2), value,
                                          value_len);
  CHECK(error == CKRYLOV_NO_SUCH_OPTION);
  CHECK(strncmp(value, "", 0) == 0);

  length = ckrylov_length_space_string_option(index, key2, strlen(key2));
