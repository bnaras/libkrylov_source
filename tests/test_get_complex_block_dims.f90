program test_get_complex_block_dims

    use kinds, only: IK
    use errors, only: OK
    use krylov, only: krylov_initialize, krylov_finalize, krylov_add_space, &
                      krylov_get_complex_block_dims, krylov_get_complex_block_total_size
    implicit none
    ...
    ! Add complex space with full dimension 10, basis dimension 3
    index = krylov_add_space('c', 'h', 'e', 10_IK, 2_IK, 3_IK)
    if (index /= 1_IK) stop 1

    ! Add second complex space with full dimension 10, basis dimension 3
    index = krylov_add_space('c', 'h', 'e', 10_IK, 2_IK, 3_IK)
    ...
    error = krylov_get_complex_block_dims(2_IK, full_dims, subset_dims, offsets)
    if (error /= OK) stop 1
...
end program test_get_complex_block_dims
