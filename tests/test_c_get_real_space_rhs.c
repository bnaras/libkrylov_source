...
int main() {
  ...
  real_t rhs_1[] = {1.0, 1.0, 1.0}, rhs_2[3];
  error = ckrylov_set_real_space_rhs(index, full_dim, solution_dim, rhs_1);
  CHECK(error == CKRYLOV_OK);

  error = ckrylov_get_real_space_rhs(index, full_dim, solution_dim, rhs_2);
  CHECK(error == CKRYLOV_OK);
  CHECK(near_real(rhs_1, rhs_2, full_dim));
