program test_solve_complex_equation
    ...
    complex(CK) :: vectors(4_IK, 1_IK), rhs(4_IK, 1_IK), solutions(4_IK, 1_IK), &
                   solutions_ref(4_IK, 1_IK), solutions1_ref(4_IK, 1_IK)
    real(RK) :: eigenvalues(1_IK), eigenvalues_ref(1_IK)
    ...
    solutions_ref = reshape((/(0.693582188120881_CK, 0.0_CK), (-0.694782745452518_CK, -0.019183995235172_CK), &
                              (0.005107696365343_CK, -0.129933915457505_CK), (0.000677699556451_CK, 0.137634621533832_CK)/), &
                            (/4_IK, 1_IK/))
    solutions1_ref = reshape((/(0.57050764852474867_CK, 0.0_CK), (-0.45076137564027469_CK, -0.0122609548410319777_CK), &
    ...
    error = krylov_solve_complex_equation(index, multiply)
    if (error /= OK) stop 1
    ...
    if (solutions /= near_complex_mat(solutions_ref, fix_phase=.true.)) stop 1
    ...
    if (solutions /= near_complex_mat(solutions1_ref, fix_phase=.true.)) stop 1
