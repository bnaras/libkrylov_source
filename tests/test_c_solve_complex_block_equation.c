...
int_t complex_block_multiply(const int_t *num_spaces, const int_t *total_size,
  ...
  complex_t matrix_1[] = {0.0, I, -I, 0.0};
  complex_t matrix_2[] = {1.0, I, -I, 1.0};
...
int main() {
  ...
  complex_t vectors_2[] = {1.0, 0.0};
  error =
      ckrylov_set_complex_space_vectors(index, full_dim, basis_dim, vectors_2);
  CHECK(error == CKRYLOV_OK);

  error = ckrylov_solve_complex_block_equation(complex_block_multiply);
  CHECK(error == CKRYLOV_OK);
  ...
  complex_t solutions_1[2], solutions_ref_1[] = {-sqrt(0.5) * I, -sqrt(0.5)};
  error = ckrylov_get_complex_space_solutions(1, full_dim, solution_dim,
                                              solutions_1);
  CHECK(near_complex_with_phase(solutions_1, solutions_ref_1,
  ...
  complex_t solutions_2[2], solutions_ref_2[] = {-sqrt(0.5) * I, -sqrt(0.5)};
  error = ckrylov_get_complex_space_solutions(2, full_dim, solution_dim,
                                              solutions_2);
  CHECK(near_complex_with_phase(solutions_2, solutions_ref_2,
                                full_dim * solution_dim, true));
