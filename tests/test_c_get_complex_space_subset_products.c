...
int main() {
  ...
  complex_t products_1[] = {I, 0.0, 0.0}, products_2[3];
  error = ckrylov_set_complex_space_subset_products(index, full_dim, skip_dim,
                                                    subset_dim, products_1);
  CHECK(error == CKRYLOV_OK);

  error = ckrylov_get_complex_space_subset_products(index, full_dim, skip_dim,
                                                    subset_dim, products_2);
  CHECK(error == CKRYLOV_OK);
  CHECK(near_complex(products_1, products_2, full_dim * subset_dim));
