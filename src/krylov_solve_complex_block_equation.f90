function krylov_solve_complex_block_equation(multiply) result(error)
    ...
    do iter = 1_IK, max_iterations
        ...
        err = krylov_set_complex_block_products(num_spaces, total_size, full_dims, subset_dims, offsets, products)
        if (err /= OK) then
            error = err
            exit
        end if
