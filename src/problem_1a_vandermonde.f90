...
! contains the matrix problem, read in from file
  type(base), allocatable :: krylov_a(:,:), a(:,:), ta(:,:)
...
!! dummy indexes
  integer(kind_integer) :: j,k,w,x,y,z = 0
...
!! creating the vandermonde matrix into krylov_a
  krylov_a = real(0,kind=kind_float)
  krylov_a(1, 1) = (real(1,kind=kind_float))
  do j = 2, n
    do k = 1, n
      x = j
      y = k - 1
      w = j - 1
      z = k
      a(j, k) = (real(x,kind=kind_float)) ** (real(y,kind=kind_float))
      ta(j, k) = (real(z,kind=kind_float)) ** (real(w,kind=kind_float))
      krylov_a(j, k) = (a(j, k) + ta(j, k))*(real(0.5,kind=kind_float)) 
    end do
  end do
