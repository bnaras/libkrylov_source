function krylov_real_nks_orthonormalizer_prepare_transform(orthonormalizer, full_dim, basis_dim, vectors) result(error)
    ...
    use lapackwrapper, only: real_potrf
    ...
    ! Compute Gram matrix
    call real_gemm('t', 'n', basis_dim, basis_dim, full_dim, 1.0_RK, vectors, full_dim, &
                   vectors, full_dim, 0.0_RK, orthonormalizer%gram_matrix, basis_dim)

    do vec = 1_IK, basis_dim
        orthonormalizer%vector_norm_squared(vec) = orthonormalizer%gram_matrix(vec, vec)
        if (orthonormalizer%vector_norm_squared(vec) < thr_zero**2) then
    ...
    err = real_sy_diag_scale(orthonormalizer%scaled_gram_matrix, orthonormalizer%vector_norm_squared, basis_dim)
    if (err /= OK) then
        error = err
        return
    end if
    ...
    info = 0_IK
    call real_potrf(uplo, basis_dim, orthonormalizer%gram_matrix_decomposed, basis_dim, info)
    if (info /= 0_IK) then
