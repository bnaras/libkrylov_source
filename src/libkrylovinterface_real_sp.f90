...
!! using an approximate spectra and frequencies as input
  type, abstract :: libkrylov_precon_real_sp
  contains
    procedure(libkrylov_precon_interface), deferred :: lkl_precon
  end type libkrylov_precon_real_sp
  abstract interface
    subroutine libkrylov_precon_interface(data,n1,n2,n3,approx_spectra,&
  &   precon_roots,full_solutions,residuals,ierr)
      import :: lkl_real_sp_k, lkl_int_rsp_k , libkrylov_precon_real_sp
      class(libkrylov_precon_real_sp) :: data
...
!!    approximate spectra
      real(lkl_real_sp_k), intent(in) :: approx_spectra(n1)
!!    approximate spectra
      real(lkl_real_sp_k), intent(in) :: precon_roots(n2)
!!    basis vectors
      real(lkl_real_sp_k), intent(in) :: full_solutions(n1,n2)
!!    residuals
      real(lkl_real_sp_k), intent(inout) :: residuals(n1,n2)
!!    error variable
      integer(lkl_int_rsp_k), intent(inout) :: ierr
...
!! defining input function with all options
  type, extends(libkrylov_precon_real_sp) :: lkl_pc_all_rsp
! external data required for the function
!! string indicating which preconditioner
    character(len=32) :: precon_string = 'davidson'
  contains
    procedure :: lkl_precon => lkl_precon_all_rsp
  end type lkl_pc_all_rsp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_real_sp) :: lkl_pc_none_rsp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_none_rsp
  end type lkl_pc_none_rsp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_real_sp) :: lkl_pc_approx_rsp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_approx_rsp
  end type lkl_pc_approx_rsp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_real_sp) :: lkl_pc_davidson_rsp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_davidson_rsp
  end type lkl_pc_davidson_rsp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_real_sp) :: lkl_pc_sleijpen_rsp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_sleijpen_rsp
  end type lkl_pc_sleijpen_rsp
...
!--------------------------------------------------------------------
  subroutine lkl_precon_none_rsp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_none_rsp
!--------------------------------------------------------------------


!--------------------------------------------------------------------
  subroutine lkl_precon_approx_rsp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_approx_rsp
!--------------------------------------------------------------------


!--------------------------------------------------------------------
  subroutine lkl_precon_davidson_rsp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
! External data (IDEALLY EMPTY)
!--------------------------------------------------------------------
    class(lkl_pc_davidson_rsp) :: data
!--------------------------------------------------------------------
! Input Parameters
!--------------------------------------------------------------------
!!    rows of residuals, nbasis
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_davidson_rsp
!--------------------------------------------------------------------

!--------------------------------------------------------------------
  subroutine lkl_precon_sleijpen_rsp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
! External data (IDEALLY EMPTY)
!--------------------------------------------------------------------
    class(lkl_pc_sleijpen_rsp) :: data
!--------------------------------------------------------------------
! Input Parameters
!--------------------------------------------------------------------
!!    rows of residuals, nbasis
...
!--------------------------------------------------------------------
    integer(lkl_int_rsp_k) :: j,k = 0
    real(lkl_real_sp_k) :: numerator
    real(lkl_real_sp_k) :: denominator
    real(lkl_real_sp_k), external :: sdot
    real(lkl_real_sp_k), allocatable :: mx(:,:)
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_sleijpen_rsp
!--------------------------------------------------------------------

!--------------------------------------------------------------------
  subroutine lkl_precon_all_rsp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_all_rsp
!
!--------------------------------------------------------------------
!--------------------------------------------------------------------
