...
!! the products of a part of the problem matrix with a set of basis vectors
  type, abstract :: libkrylov_mvprod_real_sp
  contains
    procedure(libkrylov_mvprod_intrfc_rsp), deferred :: lkl_mvp
  end type libkrylov_mvprod_real_sp
  abstract interface
    subroutine libkrylov_mvprod_intrfc_rsp(data,n1,n2,basis_vectors,&
  &   mvproduct,ierr)
      import :: lkl_int_rsp_k, libkrylov_mvprod_real_sp, lkl_real_sp_k
      class(libkrylov_mvprod_real_sp) :: data
!!    rows of guess vectors, nbasis
      integer(lkl_int_rsp_k), intent(in) :: n1
...
!!    error variable
      integer(lkl_int_rsp_k), intent(inout) :: ierr
    end subroutine libkrylov_mvprod_intrfc_rsp
  end interface
...
!! defining input function for initial basis vectors
  ...
  type, extends(libkrylov_mvprod_real_sp) :: lkl_mvp_n_mul_rsp
! external data required for the function
...
! shared with kl_approx, must be set before calling solver
    real(lkl_real_sp_k), pointer :: matrix(:,:) => null()
  contains
...
!! to determine parameters of the problem to be solved
  type :: libkrylov_problem_a_input_rsp
    integer(lkl_int_rsp_k) :: nbasis
    integer(lkl_int_rsp_k) :: nroots
    integer(lkl_int_rsp_k) :: minstart
    integer(lkl_int_rsp_k) :: nstart
    integer(lkl_int_rsp_k) :: maxstart
    real(lkl_real_sp_k) :: threshold
    integer(lkl_int_rsp_k) :: maxiter
    integer(lkl_int_rsp_k) :: totalmaxiter
    character(len=22) :: id_string
    character(len=32) :: precon_string
    integer(lkl_int_rsp_k) :: iverb
    integer(lkl_int_rsp_k) :: irestart
    real(lkl_real_sp_k), allocatable :: approx_spectra(:)
  end type libkrylov_problem_a_input_rsp

!! type for krylov_output function of problem_a
!! function that takes the output from the solver
  type :: libkrylov_problem_a_output_rsp
    type(base_rsp), allocatable :: solutions(:,:)
!!  eigenvalues
...
!!  residual norm of all vectors
  ...
  end type libkrylov_problem_a_output_rsp

!! function to determining parameters of the problem to be solved
  type :: libkrylov_problem_b_input_rsp
    integer(lkl_int_rsp_k) :: nbasis
    ...
    integer(lkl_int_rsp_k) :: irestart
    real(lkl_real_sp_k), allocatable :: approx_spectra(:)
    type(base_rsp), allocatable :: rhs(:,:)
  end type libkrylov_problem_b_input_rsp

!! type for krylov_output function of problem_b
!! that takes the output from the solver
  type :: libkrylov_problem_b_output_rsp
    type(base_rsp), allocatable :: solutions(:,:)
!!  functional
    type(base_rsp) :: lagrangian
!!  residual norm of all vectors
    real(lkl_real_sp_k) :: fro_norm
  end type libkrylov_problem_b_output_rsp

!! type for krylov_problem function for problem_c
!! function to determining parameters of the problem to be solved
  type :: libkrylov_problem_c_input_rsp
    integer(lkl_int_rsp_k) :: nbasis
    integer(lkl_int_rsp_k) :: nomega
    integer(lkl_int_rsp_k) :: nrhs
    ...
    integer(lkl_int_rsp_k) :: irestart
    real(lkl_real_sp_k), allocatable :: approx_spectra(:)
    real(lkl_real_sp_k), allocatable :: omega(:)
    type(base_rsp), allocatable :: rhs(:,:)
  end type libkrylov_problem_c_input_rsp

!! type for krylov_output function of problem_c
!! that takes the output from the solver
  type :: libkrylov_problem_c_output_rsp
    type(base_rsp), allocatable :: solutions(:,:)
!!  funtional
    type(base_rsp) :: lagrangian
!!  residual norm of all vectors
    real(lkl_real_sp_k) :: fro_norm
  end type libkrylov_problem_c_output_rsp
...
!--------------------------------------------------------------------
!--------------------------------------------------------------------
  subroutine lkl_constr_a_1_rsp(nbasis,nroots,problem_a,&
   &   output_a)
!--------------------------------------------------------------------
!--------------------------------------------------------------------
!
...
!--------------------------------------------------------------------
!!   nbasis
    integer(lkl_int_rsp_k), intent(in) :: nbasis
!!   nroots
    integer(lkl_int_rsp_k), intent(in) :: nroots
!--------------------------------------------------------------------
! Output Parameters
!--------------------------------------------------------------------
!!  problem parameters
    class(libkrylov_problem_a_input_rsp), intent(out) :: problem_a
!!  output parameters
    class(libkrylov_problem_a_output_rsp) :: output_a
!--------------------------------------------------------------------
!  Local Variables
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
    ...
    problem_a%threshold = real(4,kind=lkl_real_sp_k)
    ...
    problem_a%approx_spectra = real(0,kind=lkl_real_sp_k)
    allocate(output_a%solutions(nbasis,nroots))
    output_a%solutions%element = real(0,kind=lkl_real_sp_k)
...
!--------------------------------------------------------------------
!!  output parameters
    class(libkrylov_problem_a_input_rsp) :: problem_a
    class(libkrylov_problem_a_output_rsp) :: output_a
!--------------------------------------------------------------------
!  Local Variables
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
!--------------------------------------------------------------------
  subroutine lkl_constr_b_1_rsp(nbasis,nrhs,problem_b,&
   &   output_b)
!--------------------------------------------------------------------
!--------------------------------------------------------------------
!
...
!!   nbasis
    integer(lkl_int_rsp_k), intent(in) :: nbasis
!!   nrhs
    integer(lkl_int_rsp_k), intent(in) :: nrhs
!--------------------------------------------------------------------
! Output Parameters
!--------------------------------------------------------------------
!!  problem parameters
    class(libkrylov_problem_b_input_rsp), intent(out) :: problem_b
!!  output parameters
    class(libkrylov_problem_b_output_rsp) :: output_b
!--------------------------------------------------------------------
    ...
    problem_b%threshold = real(4,kind=lkl_real_sp_k)
    ...
    problem_b%approx_spectra = real(0,kind=lkl_real_sp_k)
    allocate(problem_b%rhs(nbasis,nrhs))
    problem_b%rhs%element = real(0,kind=lkl_real_sp_k)
    allocate(output_b%solutions(nbasis,nrhs))
    output_b%solutions%element = real(0,kind=lkl_real_sp_k)
...
!--------------------------------------------------------------------
!!  output parameters
    class(libkrylov_problem_b_input_rsp) :: problem_b
    class(libkrylov_problem_b_output_rsp) :: output_b
!--------------------------------------------------------------------
!  Local Variables
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
!--------------------------------------------------------------------
  subroutine lkl_constr_c_1_rsp(nbasis,nomega,nrhs,unique,problem_c,&
   &   output_c)
!--------------------------------------------------------------------
!--------------------------------------------------------------------
!
...
!!   nbasis
    integer(lkl_int_rsp_k), intent(in) :: nbasis
!!   nomega
    integer(lkl_int_rsp_k), intent(in) :: nomega
!!   nrhs
    integer(lkl_int_rsp_k), intent(in) :: nrhs
...
!--------------------------------------------------------------------
!!  problem parameters
    class(libkrylov_problem_c_input_rsp), intent(out) :: problem_c
!!  output parameters
    class(libkrylov_problem_c_output_rsp) :: output_c
!--------------------------------------------------------------------
!  Local Variables
!--------------------------------------------------------------------
    integer(lkl_int_rsp_k) :: nroots
!--------------------------------------------------------------------
    ...
    problem_c%threshold = real(4,kind=lkl_real_sp_k)
    ...
    problem_c%approx_spectra = real(0,kind=lkl_real_sp_k)
    allocate(problem_c%omega(nomega))
    problem_c%omega = real(0,kind=lkl_real_sp_k)
    allocate(problem_c%rhs(nbasis,nrhs))
    problem_c%rhs%element = real(0,kind=lkl_real_sp_k)
    allocate(output_c%solutions(nbasis,nroots))
    output_c%solutions%element = real(0,kind=lkl_real_sp_k)
...
!!  approximate spectra
!!  output parameters
    class(libkrylov_problem_c_input_rsp) :: problem_c
    class(libkrylov_problem_c_output_rsp) :: output_c
!--------------------------------------------------------------------
!  Local Variables
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
!--------------------------------------------------------------------
  subroutine lkl_mvp_naive_multiply_rsp(data,n1,n2,&
...
!! calculate matrix-vector product
   call sgemm('n','n',n1,n2,n1,&
  &   real(1,kind=lkl_real_sp_k),data%matrix,n1,&
  &   basis_vectors,n1,real(0,kind=lkl_real_sp_k),&
  &   mvproduct,n1)

!--------------------------------------------------------------------
!--------------------------------------------------------------------
  end subroutine lkl_mvp_naive_multiply_rsp
!--------------------------------------------------------------------
!--------------------------------------------------------------------
