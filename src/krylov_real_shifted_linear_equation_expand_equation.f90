function krylov_real_shifted_linear_equation_expand_equation(equation) result(error)
    ...
    call real_gemm('t', 'n', equation%new_dim, equation%solution_dim, equation%full_dim, &
                   1.0_RK, equation%vectors(1_IK, old_dim + 1_IK), equation%full_dim, &
                   equation%rhs, equation%full_dim, &
                   0.0_RK, equation%basis_rhs(old_dim + 1_IK, 1_IK), equation%basis_dim)
