...
!< This is the real version, double precision, using BLAS
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
    ...
    call dgemm(trans1,trans2,m,n,k,alpha%element,obj1%element,ld1, &
...
!--------------------------------------------------------------------
    ...
    obj3%element = ddot(n,obj1%element,inc1,obj2%element,inc2)
...
!--------------------------------------------------------------------
    ...
    call dpotrf(uplo,n,obj1%element,ld1,ierr)
...
!--------------------------------------------------------------------
    ...
    call dgetrf(m,n,obj1%element,ld1,ipiv,ierr)
...
!--------------------------------------------------------------------
    ...
    call dtrsm(side,uplo,trans1,diag,m,n,alpha%element, &
...
!! call LAPACK routine
    onorm = dlansy(norm,uplo,n,obj1%element,ld1,work)
...
!! solve for number desired
    call dpocon(uplo,n,obj1%element,ld1,anorm,rcond,work,iwork,ierr)
...
!! first call to LAPACK for optimal lwork
    call dsyev(jobz,uplo,n,obj1(:,:)%element,ld1,obj2(:), &
  &     lworker,-1,ierr)
...
!! Output Parameters
!!--------------------------------------------------------------------
!!! output symmetric matrix (intent out for LAPACK)
!    type(base), intent(inout) :: obj2(:,:)
!!--------------------------------------------------------------------
!! Error Parameter
!!--------------------------------------------------------------------
...
!    work_val = 2*n
!    allocate(work(work_val))
!    call dlagsy(n,k,obj1,obj2(:,:)%element,ld2, &
!  &     iseed,work,ierr) 
!
!    deallocate(work)
!
...
!! first call to LAPACK for optimal lwork
    call dsysv(uplo,n,nrhs,obj1(:,:)%element,ld1,ipiv,&
  &     obj2(:,:)%element,ld2, &
  &     lworker,-1,ierr)
