...
! frequencies
  real(kind_float), allocatable :: precon_roots(:)
! residuals
...
!! every element after is increased by 0.1
  ...
  approx_spectra(1) = real(1,kind=kind_float)
  do j1 = 2, n1
    approx_spectra(j1) = approx_spectra(j1-1) + real(0.1,kind=kind_float)
  end do
...
!! assign input residuals as real number 1
  precon_roots = real(0.5,kind=kind_float)
  residuals = real(1,kind=kind_float)
...
!! call lkl_precon_none on test input
  ...
  & precon_roots,full_solutions%element,residuals%element,ierr)
!! check ierr value to see if the subroutine terminated with an error
!! test if ierr is not equal to 0
  if (ierr.ne.0) then
...
!! assign the inverse of approx_spectra to r_ref2
     do j2 = 1, n4
      do j1 = 1, n1
        r_test2 = residuals(j1,j2)
        r_ref2 = real(1,kind=kind_float) / approx_spectra(j1)
