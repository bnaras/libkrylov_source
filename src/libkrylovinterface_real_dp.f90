...
!! using an approximate spectra and frequencies as input
  type, abstract :: libkrylov_precon_real_dp
  contains
    procedure(libkrylov_precon_interface), deferred :: lkl_precon
  end type libkrylov_precon_real_dp
  abstract interface
    subroutine libkrylov_precon_interface(data,n1,n2,n3,approx_spectra,&
  &   precon_roots,full_solutions,residuals,ierr)
      import :: lkl_real_dp_k, lkl_int_rdp_k , libkrylov_precon_real_dp
      class(libkrylov_precon_real_dp) :: data
...
!!    approximate spectra
      real(lkl_real_dp_k), intent(in) :: approx_spectra(n1)
!!    approximate spectra
      real(lkl_real_dp_k), intent(in) :: precon_roots(n2)
!!    basis vectors
...
!! for problem a
  ...
  end type libkrylov_maket_a_real_dp
  abstract interface
    subroutine libkrylov_maket_a_interface(data,n1,n2,approx_spectra,&
  &   roots,mvx,full_solutions,residuals,ierr)
      import :: lkl_real_dp_k, lkl_int_rdp_k , libkrylov_maket_a_real_dp
      class(libkrylov_maket_a_real_dp) :: data
!!    rows of residuals, nbasis
      integer(lkl_int_rdp_k), intent(in) :: n1
!!    columns of residuals, nroots
      integer(lkl_int_rdp_k), intent(in) :: n2
...
!! function to determining parameters of the problem to be solved
  type, abstract :: libkrylov_problem_b_real_dp
  contains
...
!! and does what the user wants with them
!! wheter printing or passing out of the solver
  type, abstract :: libkrylov_output_b_real_dp
  contains
...
!--------------------------------------------------------------------
  subroutine lkl_maket_a_all_rdp(data,n1,n2,approx_spectra,&
   &   roots,mvx,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
