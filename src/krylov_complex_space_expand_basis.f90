function krylov_complex_space_expand_basis(space) result(error)
    ...
    complex(CK), allocatable :: preconditioned(:, :), orthonormalized(:, :)
    ...
    err = complex_ge_subset( &
          preconditioned, active_indices, space%full_dim, space%solution_dim, active_dim)
    if (err /= OK) then
        error = err
        deallocate (active_indices, preconditioned, orthonormalized)
    end if
    ...
    err = space%orthonormalizer%prepare_vectors( &
          space%full_dim, space%basis_dim, space%solution_dim, space%equation%vectors, &
          preconditioned, orthonormalized, new_dim)
    if (err /= OK) then
        error = err
        deallocate (preconditioned, orthonormalized)
        return
