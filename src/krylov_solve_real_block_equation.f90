function krylov_solve_real_block_equation(multiply) result(error)
    ...
    use krylov, only: spaces, krylov_i_real_block_multiply, krylov_get_num_spaces, &
                      krylov_get_integer_option, krylov_get_real_block_dims, &
                      krylov_get_real_block_vectors, krylov_set_real_block_products, &
                      krylov_get_real_block_convergence, krylov_get_space_convergence
    implicit none
    ...
    do iter = 1_IK, max_iterations
        ...
        err = krylov_get_real_block_dims(num_spaces, full_dims, subset_dims, offsets, total_size)
        if (err /= OK) then
            error = err
            exit
        end if
