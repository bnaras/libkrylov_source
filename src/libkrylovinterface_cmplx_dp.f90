...
!! using an approximate spectra and frequencies as input
  type, abstract :: libkrylov_precon_cmplx_dp
  contains
    procedure(libkrylov_precon_interface), deferred :: lkl_precon
  end type libkrylov_precon_cmplx_dp
  abstract interface
    subroutine libkrylov_precon_interface(data,n1,n2,n3,approx_spectra,&
  &   precon_roots,full_solutions,residuals,ierr)
      import :: lkl_cmplx_dp_k, lkl_int_cdp_k , libkrylov_precon_cmplx_dp
      class(libkrylov_precon_cmplx_dp) :: data
...
!!    approximate spectra
      real(lkl_cmplx_dp_k), intent(in) :: approx_spectra(n1)
!!    approximate spectra
      real(lkl_cmplx_dp_k), intent(in) :: precon_roots(n2)
!!    basis vectors
      complex(lkl_cmplx_dp_k), intent(in) :: full_solutions(n1,n2)
!!    residuals
      complex(lkl_cmplx_dp_k), intent(inout) :: residuals(n1,n2)
!!    error variable
      integer(lkl_int_cdp_k), intent(inout) :: ierr
...
!! defining input function with all options
  type, extends(libkrylov_precon_cmplx_dp) :: lkl_pc_all_cdp
! external data required for the function
!! string indicating which preconditioner
    character(len=32) :: precon_string = 'davidson'
  contains
    procedure :: lkl_precon => lkl_precon_all_cdp
  end type lkl_pc_all_cdp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_cmplx_dp) :: lkl_pc_none_cdp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_none_cdp
  end type lkl_pc_none_cdp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_cmplx_dp) :: lkl_pc_approx_cdp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_approx_cdp
  end type lkl_pc_approx_cdp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_cmplx_dp) :: lkl_pc_davidson_cdp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_davidson_cdp
  end type lkl_pc_davidson_cdp

!! defining input function for null preconditioning
  type, extends(libkrylov_precon_cmplx_dp) :: lkl_pc_sleijpen_cdp
! external data required for the function
!! IDEALLY, NO EXTERNAL DATA
  contains
    procedure :: lkl_precon => lkl_precon_sleijpen_cdp
  end type lkl_pc_sleijpen_cdp
...
!--------------------------------------------------------------------
  subroutine lkl_precon_none_cdp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_none_cdp
!--------------------------------------------------------------------


!--------------------------------------------------------------------
  subroutine lkl_precon_approx_cdp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_approx_cdp
!--------------------------------------------------------------------


!--------------------------------------------------------------------
  subroutine lkl_precon_davidson_cdp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
! External data (IDEALLY EMPTY)
!--------------------------------------------------------------------
    class(lkl_pc_davidson_cdp) :: data
!--------------------------------------------------------------------
! Input Parameters
!--------------------------------------------------------------------
!!    rows of residuals, nbasis
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_davidson_cdp
!--------------------------------------------------------------------

!--------------------------------------------------------------------
  subroutine lkl_precon_sleijpen_cdp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
! External data (IDEALLY EMPTY)
!--------------------------------------------------------------------
    class(lkl_pc_sleijpen_cdp) :: data
!--------------------------------------------------------------------
! Input Parameters
!--------------------------------------------------------------------
!!    rows of residuals, nbasis
...
!--------------------------------------------------------------------
    integer(lkl_int_cdp_k) :: j,k = 0
    complex(lkl_cmplx_dp_k) :: numerator
    complex(lkl_cmplx_dp_k) :: denominator
    complex(lkl_cmplx_dp_k), external :: zdotc
    complex(lkl_cmplx_dp_k), allocatable :: mx(:,:)
!--------------------------------------------------------------------
...
!! create scaled full solutions required for epsilon
  ...
  &       ( approx_spectra(j) - precon_roots(k) )
    ...
    do k = 1, n2
      denominator = zdotc(n1,mx(1:n1,k),1,full_solutions(1:n1,k),1)
      numerator = zdotc(n1,mx(1:n1,k),1,residuals(1:n1,k),1)
      do j = 1, n1
        residuals(j,k) = ((residuals(j,k) &
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_sleijpen_cdp
!--------------------------------------------------------------------

!--------------------------------------------------------------------
  subroutine lkl_precon_all_cdp(data,n1,n2,n3,approx_spectra,&
   &   precon_roots,full_solutions,residuals,ierr)
!--------------------------------------------------------------------
!
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
    ...
    complex(lkl_cmplx_dp_k) :: numerator
    complex(lkl_cmplx_dp_k) :: denominator
    complex(lkl_cmplx_dp_k), external :: ddot
    complex(lkl_cmplx_dp_k), allocatable :: mx(:,:)
!--------------------------------------------------------------------
...
!--------------------------------------------------------------------
  end subroutine lkl_precon_all_cdp
!
!--------------------------------------------------------------------
