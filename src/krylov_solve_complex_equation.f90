function krylov_solve_complex_equation(index, multiply) result(error)
    ...
    use krylov, only: spaces, complex_space_t, krylov_i_complex_multiply, krylov_get_num_spaces, &
                      krylov_get_space_integer_option, krylov_get_complex_space_subset_vectors, &
                      krylov_set_complex_space_subset_products, krylov_get_space_convergence
    implicit none
    ...
    complex(CK), allocatable :: vectors(:, :), products(:, :)
    ...
    type is (complex_space_t)
        ...
        do iter = 1_IK, max_iterations
            ...
            err = krylov_get_complex_space_subset_vectors(index, full_dim, skip_dim, subset_dim, vectors)
            if (err /= OK) then
                error = err
                exit
            end if
            ...
            err = krylov_set_complex_space_subset_products(index, full_dim, skip_dim, subset_dim, products)
            if (err /= OK) then
                error = err
                exit
            end if
