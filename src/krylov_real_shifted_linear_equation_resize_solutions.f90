function krylov_real_shifted_linear_equation_resize_solutions(equation, solution_dim) result(error)
    ...
    real(RK), allocatable :: solutions_tmp(:, :), residuals_tmp(:, :), rhs_tmp(:, :), &
                             shifts_tmp(:), basis_solutions_tmp(:, :), basis_rhs_tmp(:, :)
    ...
    if (solution_dim > equation%solution_dim) then
        ...
        allocate (solutions_tmp(equation%full_dim, solution_dim), &
                  ...
                  rhs_tmp(equation%full_dim, solution_dim), &
                  shifts_tmp(equation%solution_dim), &
                  basis_solutions_tmp(equation%basis_dim, solution_dim), &
                  basis_rhs_tmp(equation%basis_dim, solution_dim))
        ...
        shifts_tmp(1_IK:equation%solution_dim) = equation%shifts
        ...
        call move_alloc(shifts_tmp, equation%shifts)
        ...
        deallocate (equation%solutions, equation%residuals, equation%rhs, &
                    equation%shifts, equation%basis_solutions, equation%basis_rhs)
