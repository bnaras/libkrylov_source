...
!!    obj to be interacted with
      complex(lkl_single_k), intent(inout) :: obj(n1,n2)
...
!!    approximate spectra
      real(lkl_single_k), intent(in) :: approx_spectra(n1)
!!    guess vectors
      complex(lkl_single_k), intent(inout) :: basis_vectors(n1,n2)
!!    error variable
      integer(lkl_int_k), intent(inout) :: ierr
...
!!    desired matrix vector products, mvproducts
      complex(lkl_single_k), intent(inout) :: mvproduct(n1,n2)
!!    error variable
      integer(lkl_int_k), intent(inout) :: ierr
...
!!    basis vectors
      complex(lkl_single_k), intent(in) :: full_solutions(n1,n2)
!!    residuals
      complex(lkl_single_k), intent(inout) :: residuals(n1,n2)
!!    error variable
      integer(lkl_int_k), intent(inout) :: ierr
...
!! defining input function with all options
  type, extends(libkrylov_precon_subroutine) :: lkl_pc_all
! external data required for the function
...
!!    functional
      complex(lkl_single_k), intent(in) :: lagrangian(n3)
!!    solutions on the full space, stored on mvproduct
      complex(lkl_single_k), intent(in) :: solutions(n1,n3)
...
!!    array for which solutions are converged
      logical, intent(in) :: jconverged(n3)
!!    rhs
      complex(lkl_single_k), intent(in) :: rhs(n1,n3)
!!    functional
      complex(lkl_single_k), intent(in) :: lagrangian(n3)
!!    solutions on the full space, stored on mvproduct
      complex(lkl_single_k), intent(in) :: solutions(n1,n3)
...
!!    rhs
      complex(lkl_single_k), intent(in) :: rhs(n1,n4)
!!    lagrangian
      complex(lkl_single_k), intent(in) :: lagrangian(n5)
!!    solutions on the full space, stored on mvproduct
      complex(lkl_single_k), intent(in) :: solutions(n1,n5)
!!    residual norms of each vector
...
!--------------------------------------------------------------------
    integer(lkl_int_k) :: j,k = 0
    complex(lkl_single_k) :: numerator
    complex(lkl_single_k) :: denominator
    complex(lkl_single_k), external :: cdotc
    complex(lkl_single_k), allocatable :: mx(:,:)
!--------------------------------------------------------------------
