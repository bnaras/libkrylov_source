...
!--------------------------------------------------------------------
    ...
    call zgemm(trans1,trans2,m,n,k,alpha%element,obj1%element, &
...
!--------------------------------------------------------------------
    ...
    call zhemm(side,uplo,m,n,alpha,obj1,ld1,  &
  &     obj2,ld2,beta,obj3,ld3)
...
!--------------------------------------------------------------------
    ...
    obj3%element = zdotc(n,obj1%element,inc1,obj2%element,inc2)
...
!--------------------------------------------------------------------
    ...
    call zpotrf(uplo,n,obj1%element,ld1,ierr)
...
!--------------------------------------------------------------------
    ...
    call ztrsm(side,uplo,trans1,diag,m,n,alpha%element, &
...
!--------------------------------------------------------------------
    ...
    call zpotrs(trans1,n,m,obj1%element,ld1, &
...
!! call LAPACK routine
    onorm = zlanhe(norm,uplo,n,obj1%element,ld1,work)
...
!! solve for norm desired
    call zpocon(uplo,n,obj1%element,ld1,anorm,rcond,work,rwork,ierr)
...
!! first call to LAPACK for optimal lwork
    call zheev(jobz,uplo,n,obj1(:,:)%element,ld1,obj2(:),&
...
!--------------------------------------------------------------------
   ...
   lwork_val = n
!! first call to LAPACK for optimal lwork
    call zhesv(uplo,n,nrhs,obj1(:,:)%element,ld1,ipiv,&
  &     obj2(:,:)%element,ld2, &
  &     lworker,-1,ierr)
...
!! first call to LAPACK for optimal lwork
    call zunmqr(side,translate,m,n,k,obj1(:,:)%element,ld1,&
...
!! first call to LAPACK for optimal lwork
    call zgeqrf(m,n,obj1(:,:)%element,ld1,tau(:)%element,&
  &     lworker,-1,ierr)
...
!! first call to LAPACK for optimal lwork
    call zungqr(m,n,k,obj1(:,:)%element,ld1,tau(:)%element,&
  &     lworker,-1,ierr)
...
!! first call to LAPACK for optimal lwork
    call zgesvd(jobu,jobvt,m,n,obj1(:,:)%element,ld1,s(:),&
